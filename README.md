...
\*\*Status:\*\* Core analysis complete. Dashboard live. See \[Future Work](#future-work) for planned extensions.
...
Five NHANES 2017–2018 files were pulled programmatically (Python, Google Colab) directly from the CDC's public data server: demographics (`DEMO\_J`), diabetes questionnaire (`DIQ\_J`), depression screener (`DPQ\_J`), glycohemoglobin lab (`GHB\_J`), and healthcare utilization (`HUQ\_J`). A sixth file, cholesterol/triglycerides (`TRIGLY\_J`), was added later to bring in LDL as a secondary outcome. Files were converted from SAS transport format (`.xpt`) to CSV and loaded into Google BigQuery.
...
1\. \*\*Descriptive statistics\*\* (mean, standard deviation, n per group) — computed in SQL (BigQuery).

2\. \*\*Welch's t-test\*\* (unequal-variance t-test) — chosen over the standard Student's t-test because the comorbid and non-comorbid groups differ substantially in size (roughly 90 vs. 640–740) and, on inspection, in variance as well; Welch's test is the safer default in exactly this situation.

3\. \*\*Multivariable linear regression\*\*, adjusting for age and sex, to check whether each unadjusted association held after accounting for these two basic confounders.
...
Built in Power BI (`/powerbi/dual\_burden\_dashboard.pbix`). Shows cohort sizes, the HbA1c comparison with its significance annotation, and both utilization comparisons, each captioned with its adjusted p-value.



!\[Dashboard preview](powerbi/dashboard\_preview.png)
